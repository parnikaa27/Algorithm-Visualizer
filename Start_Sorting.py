...
class Sorting:
    def __init__(self, root, AlgoNameVar):
        ...
        self.frame1 = Frame(root, width=self.FRAME1_X, height=self.FRAME1_Y, bg="#003366")
        ...
        self.algo_menu.config(font="calibri", bg="#4BAAC8", activebackground="#ADD8E6", cursor="circle")
        ...
        self.frame_btn1 = Frame(self.frame1, width=230, height=40, bg="#003366")
        self.frame_btn1.grid_propagate(0)
        self.frame_btn1.place(relx=0.0, rely=0.17)
        # Button for generating new array
        self.btn_new = Button(self.frame_btn1, text="Generate", padx=13, pady=3, command=self.new_list, bg="black", fg="azure", cursor="hand2")
        self.btn_new.place(relx=0.15, rely=0)
        # Button for shuffling the array
        self.btn_shuffle = Button(self.frame_btn1, text="Shuffle", padx=13, pady=3, command=self.shuffle_list, bg="black", fg="azure", cursor="hand2")
        self.btn_shuffle.place(relx=0.60, rely=0)


        # Option for bar / color graph
        # Creating new frame for it
        self.frame_radio = Frame(self.frame1, bg="#003366", width=230, height=25, relief="flat", bd=4)
        self.frame_radio.place(relx=0, rely=0.23)
        self.frame_radio.grid_propagate(0)
        # Creating the button / option
        self.bar_drawing = Radiobutton(self.frame_radio, text="Bar", bg="#4BAAC8", fg="navy", variable=self.graph_type, value=0,
                                       font=("Helvetica", 10, "bold"), command=self.draw_type, cursor="hand2")
        self.color_drawing = Radiobutton(self.frame_radio, text="Color", bg="#4BAAC8", fg="navy", variable=self.graph_type,
        ...
        self.frame_btn2 = Frame(self.frame1, width=230, height=40, bg="#003366")
        self.frame_btn2.grid_propagate(0)
        self.frame_btn2.place(relx=0.0, rely=0.3)
        # Creating a sort button
        self.btn_sort = Button(self.frame_btn2, text="Sort", padx=13, pady=3, command=self.sort_list, bg="black", fg="azure", cursor="hand2")
        ...
        self.scale_size = Scale(self.frame1, label="Size :", orient=HORIZONTAL, from_=10, to=200, length=230,
                                bg="white", troughcolor="#024e76", variable=self.size_var, command=self.change_size,
        ...
        self.scale_speed = Scale(self.frame1, label="Speed :", orient=HORIZONTAL, from_=1, to=500, length=230,
                                bg="white", troughcolor="#024e76", variable=self.speed_var, command=self.change_speed, relief="solid", cursor="hand2")
        ...
        self.label_comparison = Label(self.frame1, text=" No. of comparisons: 0", bg="#003366", fg="white", font=("Fixedsys", 12))
        self.label_comparison.place(relx=0.1, rely=0.65)


        # Frame for algorithm info
        self.frame_algo_info = Frame(self.frame1, bg="#7CC1D7", width=230, height=150, relief="sunken", bd=4)
        self.frame_algo_info.grid_propagate(0)
        self.frame_algo_info.place(relx=0.03, rely=0.7)
        # Label for algorithm info
        self.label_avg = Label(self.frame_algo_info, bg="#7CC1D7", fg="white", text=self.information[self.algo_var.get()], font=("comic sans ms", 13, "bold"))
        ...
        self.canva = Canvas(self.frame2, width=self.CANVAS_X, height=self.CANVAS_Y, bg="#4BAAC8")
        self.canva.pack()
        ...
        for num in self.numbers:
            self.canva.create_rectangle(self.starting_point, self.CANVAS_Y - num, self.starting_point + self.rec_width, self.CANVAS_Y, fill="#DEEFF5")
    ...
    def new_list(self):
        ...
        # if bar graph is selected
        if self.TYPE == 0:
            colortype = ["#DEEFF5" for x in self.numbers]
        # if color graph is selected
        else:
